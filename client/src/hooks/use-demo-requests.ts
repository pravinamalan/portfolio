...
import { api, type CreateDemoRequestInput } from "@shared/routes";
import { z } from "zod";
  ...
  return useMutation({
    mutationFn: async (input: CreateDemoRequestInput) => {
      ...
      if (!res.ok) {
        if (res.status === 400) {
          const err = parseWithLogging(
            api.demoRequests.create.responses[400],
            await res.json(),
            "demoRequests.create.responses[400]",
          );
      ...
      return parseWithLogging(
        api.demoRequests.create.responses[201],
        await res.json(),
        "demoRequests.create.responses[201]",
      );
    },
    onSuccess: async () => {
